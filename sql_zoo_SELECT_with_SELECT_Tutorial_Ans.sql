...
FROM world
WHERE continent = 'South America' OR name='Australia'
...
/*
Question 8:
List each continent and the name of
the country that comes first alphabetically.
*/
...
WHERE 25000000 >=  ALL(SELECT population
                       FROM world x
                       WHERE y.continent = x.continent AND x.population > 0)
...
WHERE population > ALL(SELECT population*3
                       FROM world x
                       WHERE y.continent = x.continent AND x.population > 0 AND x.name != y.name)
